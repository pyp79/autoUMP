...
def main():
    ...
    for row in rows:
        ip_address = row[0]
        if is_vip(ip_address):
            ip_list = vip_to_hostip(ip_address)
...
def is_vip(ip_address):
    conn = get_conn()
    cursor = conn.cursor()
    sql = "select count(*) from {0} where VIP='{1}'".format(IP_TABLE,ip_address)
    cursor.execute(sql)
    rows = cursor.fetchall()
    if rows[0][0] > 0:
        print(ip_address, rows)
        return True

    else:
        return False

def vip_to_hostip(ip_address):
    conn = get_conn()
    cursor = conn.cursor()
    sql = "select PHYSICIP from {0} where VIP='{1}'".format(IP_TABLE,ip_address)
    cursor.execute(sql)
    rows = cursor.fetchall()
    ip_list = []
    for row in rows:
...
def mapip_to_hostip():
    ...
    conn = get_conn()
    cursor = conn.cursor()
    sql = "select MAPPINGIP,PHYSICIP from {0} where MAPPINGIP != 'None'".format(IP_TABLE)
    cursor.execute(sql)
    rows = cursor.fetchall()
    for row in rows:
